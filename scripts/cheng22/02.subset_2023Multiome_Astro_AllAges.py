...
OUT_FILE = os.path.join(DATA_DIR, 'superdupermegaRNA_yoo25_Astro_AllAges.h5ad')
...
mask = (adata.obs['Study'] == STUDY) & (adata.obs['Subclass'].isin(SUBCLASSES))
adata_sub = adata[mask].to_memory()
print(f'Subset (Study=={STUDY!r}, all ages, Subclass in {SUBCLASSES}): {adata_sub}')
