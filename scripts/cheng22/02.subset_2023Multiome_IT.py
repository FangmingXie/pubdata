...
OUT_FILE = os.path.join(DATA_DIR, 'superdupermegaRNA_yoo25_IT_P21.h5ad')
...
mask = (adata.obs['Study'] == STUDY) & (adata.obs['Age'] == AGE) & (adata.obs['Subclass'].isin(SUBCLASSES))
adata_sub = adata[mask].to_memory()
print(f'Subset (Study=={STUDY!r}, Age=={AGE!r}, Subclass in {SUBCLASSES}): {adata_sub}')
